...
void initializeBuffers(uint8_t *tx, uint8_t *rx, size_t size, size_t offset, uint8_t initial_value)
{
    memset(tx, initial_value, size);
    memset(rx, initial_value, size);
}

void clearBuffers(uint8_t *buff, size_t size)
{
